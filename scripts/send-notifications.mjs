...
// Conserve uniquement les balises <strong> de l'app, neutralise tout le reste
function safeHtml(s) {
  return String(s || '').split(/(<\/?strong>)/i)
    .map(part => /^<\/?strong>$/i.test(part) ? part.toLowerCase() : escapeHtml(stripHtml(part)).replace(/\n/g, '<br>'))
    .join('');
}
function appUrl() {
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f4f6f7;padding:24px 12px;"><tr><td align="center">
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#ffffff;border-radius:12px;border:1px solid #e6e6e6;">
      ...
      <tr><td style="padding:22px 24px;font-size:15px;line-height:1.6;">
        ...
        ${bodyHtml}
        ${url ? `<div style="margin-top:22px;"><a href="${escapeHtml(url)}" style="display:inline-block;background:#25737d;color:#ffffff;text-decoration:none;padding:10px 18px;border-radius:6px;font-weight:600;font-size:14px;">Ouvrir l'application Congés</a></div>` : ''}
      </td></tr>
...
async function main() {
  ...
  for (const n of [...fresh].sort((a, b) => (a.ts || 0) - (b.ts || 0))) {
    ...
    const subject = `${KIND_LABELS[n.kind] || 'Congés RH'} — ${summary}`;
    const message = {
      from,
      to: to.join(', '),
      subject,
